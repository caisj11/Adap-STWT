...
from utils.tools import get_mae, get_mape, get_rmse, get_r2
...
class TFTrainer(Trainer):
    def __init__(self, model, optimizer, lr_scheduler, max_epoch_num, scaler, model_save_path):
        ...
        self.early_stop = EarlyStop(40, min_is_best=True)
    ...
    def evaluate(self, data_loader, metrics, **kwargs):
        ...
        for x_enc, x_enc_mark, x_dec, x_dec_mark, traj_mx, y in data_loader:
            ...
            pred = self.model(x_enc, x_enc_mark, x_dec, x_dec_mark, **kwargs).detach()
    ...
    @torch.no_grad()
    def evaluate_graph(self, graph_model, data_loader, metrics, adj_mx, teacher_forcing_ratio):
        self.model.eval()
        graph_model.eval()
        y_true, y_pred, loss, data_num = [], [], 0, 0
        for x, traj, time, y in data_loader:
            x = x.type(torch.float32).to(self.device)
            y = y.type(torch.float32).to(self.device)
            traj = traj.type(torch.float32).to(self.device)
            new_adj_mx = graph_model(x, adj_mx, traj).detach()
            pred = self.model(x, y, new_adj_mx, teacher_forcing_ratio).detach()
            loss += self.model_loss_func(pred, y).item()
            data_num += len(data_loader)
            y_true.append(y)
            y_pred.append(pred)
